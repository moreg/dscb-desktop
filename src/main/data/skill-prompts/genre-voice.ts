...
export const GENRE_VOICES: readonly GenreVoice[] = [
  {
    ...
    // 只列不撞系统铁律 10 禁用清单的虚词——「仿佛/一丝/一抹」在铁律 10 里被点名禁，
    // 若题材又放行，模型收到互相矛盾的指令只会随机遵守一边
    allowedHedges: ['渐渐', '一时之间', '似乎'],
  ...
  },
  {
    ...
    // 同 xianxia：不列撞系统铁律 10 禁用清单的词（「一丝/一抹」被点名禁）
    allowedHedges: ['似乎', '渐渐'],
    replacements: [
      ['嘴角带了点弧度', '笑了一下'],
      // 铁律 6 允许"偶用且情境贴切"的套路词，此处保留单字推荐但不与"频繁滥用必替换"冲突
      ['心跳慢了一拍', '顿了顿'],
      ['气氛微妙', '无人说话'],
      ['隐约觉得不对', '皱了下眉'],
      ['天边泛起鱼肚白', '天刚亮'],
