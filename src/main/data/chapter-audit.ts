...
import type { AuditCategory, AuditReport, AuditSeverity, AuditViolation } from '../../shared/types'
...
export function auditChapter(content: string, opts: AuditOptions = {}): AuditReport {
  ...
  pushWordCountViolation(content, minWords, maxWords, violations)
...
function computePassed(violations: AuditViolation[]): AuditReport['passed'] {
  const errored: Record<AuditCategory, boolean> = {
    ending: false,
  ...
  for (const v of violations) {
    if (v.severity === 'error') errored[v.category] = true
  }
  return {
